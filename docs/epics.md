...
| Epic 13 | #72 Presentation Runtime and Physics Stability | Architecture ready | Single-branch delivery on `agent/epic-13-presentation-runtime`: physics-debug opt-in (query + F4), art-ready actor-view seam with stable authoritative bodies, `actor-art.json` catalog + Aseprite-produced proving sprites for all five current actors plus one projectile and one pickup, charger/rusher environment clipping, pause/Retry presentation pinning; architecture in [`architecture/epic-13-presentation-runtime.md`](architecture/epic-13-presentation-runtime.md). |
