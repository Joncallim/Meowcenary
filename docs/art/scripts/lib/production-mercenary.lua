...
local function boar(s,f,C)
  local b,st,fall=pose(f); local r=20+b+fall; local i=U.clearCel(s,"body",f).image
...
local function cougar(s,f,C)
  local b,st,fall=pose(f); local r=18+b+fall; local i=U.clearCel(s,"body",f).image
...
local function weasel(s,f,C)
  local b,st,fall=pose(f); local r=17+b+fall; local i=U.clearCel(s,"body",f).image
...
local function ram(s,f,C)
  ...
  U.outlinedRect(i,8,r+7,7,15,C.metal); U.outlinedRect(i,34,r+7,7,15,C.metal); U.outlinedEllipse(i,11-st,r+23,5,4,C.dark); U.outlinedEllipse(i,38+st,r+23,5,4,C.dark); U.outlinedRect(i,18+st,38,5,4,C.dark); U.outlinedRect(i,27-st,38,5,4,C.dark)
