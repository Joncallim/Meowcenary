...
if (width !== 160 || height !== 32 || !Array.isArray(project.layers) || !Array.isArray(project.frames) || project.frames.length !== 1) throw new Error('Commando source must be one 160x32 Pixelorama frame');
const output = Buffer.alloc(width * height * 4);
const alphaComposite = (destination, sourcePixels) => {
  for (let index = 0; index < sourcePixels.length; index += 4) {
    const alpha = sourcePixels[index + 3] / 255;
    if (alpha === 0) continue;
    const under = destination[index + 3] / 255; const outAlpha = alpha + under * (1 - alpha);
...
for (const [index, layer] of project.layers.entries()) {
  ...
  if (raw.length !== output.length) throw new Error(`Invalid RGBA cel for Commando layer ${index + 1}`);
