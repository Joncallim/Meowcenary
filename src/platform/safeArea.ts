...
export function clampSafeAreaInsets(
  insets: SafeAreaInsetsPx,
  containerWidth: number,
  containerHeight: number,
): SafeAreaInsetsPx {
  const width = positiveFinite(containerWidth);
  const height = positiveFinite(containerHeight);
  return Object.freeze({
    top: Math.min(finiteNonNegative(insets.top), height),
    right: Math.min(finiteNonNegative(insets.right), width),
    bottom: Math.min(finiteNonNegative(insets.bottom), height),
    left: Math.min(finiteNonNegative(insets.left), width),
  });
}
