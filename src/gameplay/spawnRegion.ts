...
export function spawnPoint(arena: Readonly<ArenaDefinition>, rng: Rng): Vec2 {
  ...
  return fallbackPoint(region, arena);
}
...
function fallbackPoint(region: SpawnRegion, arena: Readonly<ArenaDefinition>): Vec2 {
  ...
  switch (region.kind) {
    case 'ring': {
      ...
      // Sweep in finer increments around the ring
      const steps = 16;
      for (let i = 0; i < steps; i += 1) {
        const a = (2 * Math.PI * i) / steps;
        const radii = [midR, region.minRadius, region.maxRadius, (region.minRadius + midR) / 2, (midR + region.maxRadius) / 2];
        for (const r of radii) {
          const px = region.cx + r * Math.cos(a);
          const py = region.cy + r * Math.sin(a);
          const p = { x: px, y: py };
          if (isValidPoint(p, region, arena)) return p;
        }
      }
      // Last resort: clamped midpoint
      const x = Math.max(0, Math.min(width, region.cx + midR));
      return { x, y: Math.max(0, Math.min(height, region.cy)) };
    }
    case 'rect': {
      const step = Math.min(region.w, region.h, 10);
      for (let sx = region.x + 1; sx <= region.x + region.w - 1; sx += step) {
        for (let sy = region.y + 1; sy <= region.y + region.h - 1; sy += step) {
          const p = { x: sx, y: sy };
          if (isValidPoint(p, region, arena)) return p;
        }
      }
