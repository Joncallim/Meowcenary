...
/** V4 dedicated Equipment art is generic `icon` presentation. Existing rows
 * temporarily retain their explicitly borrowed run-upgrade identity until
 * their own production families replace it; no fake alias art is introduced. */
export function assertEquipmentArtReferences(pieces: readonly EquipmentDefinition[], sets: readonly EquipmentSetDefinition[], catalog: VisualArtCatalog): void { const bindings = new Map(catalog.bindings.map((binding) => [binding.id, binding])); for (const [kind, id] of [...pieces.map((piece) => ['equipment', piece.icon] as const), ...sets.map((set) => ['equipment set', set.emblem] as const)]) { const binding = id === undefined ? undefined : bindings.get(id); const validKind = binding?.kind === 'icon' || binding?.kind === 'upgrade-icon'; if (!binding || !validKind || !binding.required) throw new Error(`${kind}: unknown required icon "${id}"`); } }
