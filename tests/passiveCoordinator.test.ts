...
describe('PassiveCoordinator', () => {
  it('installs subscriptions and dispatches on declared event', () => {
    ...
    bus.emit('enemy:killed', { instanceId: 1, enemyId: 'd', xpValue: 10, x: 0, y: 0 });
  ...
  it('blocks reentrancy and logs an error', () => {
    ...
    const handler: PassiveHandler<CharacterPassiveEvent> = (_ctx, _payload) => {
      bus.emit('enemy:killed', { instanceId: 3, enemyId: 'e', xpValue: 20, x: 10, y: 10 });
      reentered = true;
    };
  ...
  it('unsubscribes from events on destroy', () => {
    ...
    coord.destroy();
    bus.emit('enemy:killed', { instanceId: 2, enemyId: 'e', xpValue: 20, x: 10, y: 10 });
    expect(callCount).toBe(1);
  });
  ...
  it('suppresses dispatch when RunState is not active', () => {
    ...
    run.status = 'active';
    bus.emit('enemy:killed', { instanceId: 3, enemyId: 'f', xpValue: 30, x: 20, y: 20 });
    expect(callCount).toBe(1);
